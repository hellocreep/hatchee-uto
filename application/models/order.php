...
class Order extends CI_Model
{
	...
	public function search($id,$type)
	{
		...
		if($type=="order")
		{
			$sql='select uto_inquiry.Id as id,uto_inquiry.user as uid,uto_inquiry.tour as tid,uto_inquiry.status as status,uto_inquiry.uuid as orderid,uto_inquiry.people as num,uto_inquiry.create_date as ordertime,uto_inquiry.is_worked,uto_inquiry.handle_time,uto_users.name as uto_username,uto_tour.name as tourname from uto_inquiry,uto_users,uto_tour where uto_inquiry.user=uto_users.id and uto_inquiry.tour=uto_tour.id and uto_inquiry.Id="'.$id.'"';	
		}
		else
		{
			$sql="select c.Id as id, c.uuid as orderid,c.create_date as ordertime,c.is_worked,c.handle_time,c.tour_time as tourtime,u.name as username,u.Id as uid from uto_custom_inquiry as c left join uto_users as u on c.user=u.Id where c.uuid='".$id."'";
		}
		$query=$this->db->query($sql);
		$data=$query->result();
		return $data;
