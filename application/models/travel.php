...
class Travel extends CI_Model
{
	...
	{
		$start=($page-1)*$step;
		$sql = "select Id, title, edit_time from uto_travel_note limit ".$start.",".$step;
		$query=$this->db->query($sql);
		return $query->result_array();
	}

	public function notecount(){
		$sql = "select Id from uto_travel_note";
		$query = $this->db->query($sql);
		$data = $query->num_rows();
		return $data;
	}
	...
	{
		$start=($page-1)*$step;
		$sql="select * from uto_travel_note limit ".$start.",".$step;
		$query=$this->db->query($sql);
		return $query->result_array();
	}
	...
	{
		$this->db->insert('uto_travel_note',$data);
		return $this->db->insert_id();
	}
	public function update($id,$data)
	{
		$this->db->where('Id',$id);
		$this->db->update('uto_travel_note',$data);
		return $this->db->affected_rows();
	}
	public function del($id)
	{
		$this->db->where("Id",$id);
        $this->db->delete("uto_travel_note");
		return $this->db->affected_rows();	
	}
