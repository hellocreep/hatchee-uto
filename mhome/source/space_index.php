...
//订单
	...
	$query1 = $_SGLOBAL['db']->query("select count(*) from uto_inquiry where user='$user[Id]'");
	$count1=$_SGLOBAL['db']->result($query1);
	$query2 = $_SGLOBAL['db']->query("select count(*) from uto_custom_inquiry where user='$user[Id]'");
	$count2=$_SGLOBAL['db']->result($query2);
	$ordernum=$count1+$count2;
	$space['ordernum']=$ordernum;
