<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/mtag|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1343371784', 'admin/tpl/mtag');?><?php $_TPL['menunames'] = array(
		'index' => '管理首页',
		'config' => '站点设置',
		'privacy' => '隐私设置',
		'usergroup' => '用户组',
...
<option value="1"<?php if($_GET['close']==1) { ?> selected<?php } ?>>锁定</option>
</select>
</td>
<th>是否推荐</th><td>
<select name="recommend">
<option value="">不限制</option>
<option value="1"<?php if($_GET['recommend']==1) { ?> selected<?php } ?>>推荐</option>
</select>
</td>
</tr>
...
<option value="<?=$value['fieldid']?>"<?php if($_GET['fieldid']==$value['fieldid']) { ?> selected<?php } ?>><?=$value['title']?></option>
<?php } } ?>
</select>
</td>
</tr>
...
<tr>
<td><input type="<?php if($managebatch) { ?>checkbox<?php } else { ?>radio<?php } ?>" name="ids[]" value="<?=$value['tagid']?>">
<a href="space.php?do=mtag&tagid=<?=$value['tagid']?>" target="_blank"><?=$value['tagname']?></a>
<?php if($value['close']) { ?>(<a href="admincp.php?ac=mtag&close=<?=$value['close']?>">锁定</a>)<?php } ?>
<?php if($value['recommend']) { ?>(<a href="admincp.php?ac=mtag&recommend=<?=$value['recommend']?>">推荐</a>)<?php } ?>
</td>
