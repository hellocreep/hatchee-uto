<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/space_poll_list|template/default/header|template/default/space_menu|template/default/footer', '1343372290', 'template/default/space_poll_list');?><?php $_TPL['titles'] = array('投票'); ?>
...
<p><input type="<?php if($value['multiple']) { ?>checkbox<?php } else { ?>radio<?php } ?>" disabled name="poll_<?=$value['pid']?>"/> <?=$val?></p>
 <?php } } ?>
</div>
</li>
<?php } } ?>
...
<?php if($space['friendnum']) { ?><li<?=$actives['friend']?>><a href="space.php?uid=<?=$space['uid']?>&do=<?=$do?>&view=friend"><span>好友投票</span></a></li><?php } ?>
...
<?php if($space['self'] && in_array($_GET['view'], array('me', 'hot'))) { ?>
<div class="h_status">
...
<h4>
<?php if($value['percredit']) { ?><span class="poll_reward">+<?=$value['percredit']?></span><?php } ?><a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&pid=<?=$value['pid']?>"><?=$value['subject']?></a>
</h4>
<div class="poll_options">
<ol>
 <?php if(is_array($value['option'])) { foreach($value['option'] as $val) { ?>
<li><input type="<?php if($value['multiple']) { ?>checkbox<?php } else { ?>radio<?php } ?>" disabled name="poll_<?=$value['pid']?>"/> <?=$val?></li>
 <?php } } ?>
<li>……</li>
 </ol>
 <p>
 	...
 	<span class="pipe">|</span>
 	<span class="gray">投票已结束</span><?php if(empty($value['summary'])) { ?>&nbsp;<a class="green" href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&pid=<?=$value['pid']?>">去写写投票总结</a><?php } ?>
 	<?php } ?>
 </p>
</div>
</div>
<div class="poll_status">
<a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&pid=<?=$value['pid']?>" class="poll_joins pj_<?php echo random(1, 1) ?>"><span><?=$value['voternum']?></span>人参与</a> <a href="space.php?uid=<?=$value['uid']?>&do=<?=$do?>&pid=<?=$value['pid']?>" class="go2_poll">去投票</a>
</div>
</li>
<?php } } ?>
